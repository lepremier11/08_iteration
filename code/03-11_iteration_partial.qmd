...
Imagery data was extracted from each point where soil and plant samples were taken. Imagery data included reflectance in the bands of   
  ...
  - near-infrared (nir),  
  - red-edge (re)  
Healthy plants reflect little of r, g, b as they absorb a lot and reflect a lot of near-infrared
# Tasks  
...
# pivot longer just take wide to columns and stack them together 
...
```
```{r}
ggplot(data= nematode_w, aes(x = sampleid, y = mel_s)
       ) +
  geom_point()
```
```{r}
...
```{r}
for (i in 3:ncol(nematode_w)) {  # i = index position;3: starting column the "for" loop will consider
...
varname <- colnames[k]
  # Calculating mean and cv; double bracket will pull vector from a column
statsum <- meancv(nematode_w[[k]]) 
  # Appending results to the empty data frame  
  nematode_meancv$variable[[k-2]] <- varname
  nematode_meancv$mean[[k-2]] <- round(statsum$mean, 3)
  nematode_meancv$cv[[k-2]] <- round(statsum$cv, 3)
  # Print a message
...
pivot_longer(cols = b:nli,
  ...
  # Creating groups of resp_var and exp_var , the result after only nest only, you will see <tibble> which is the dataframe related to one combination of Resp~expl 
 group_by(resp_var, exp_var) %>%
  nest() %>%
  # Applying linear model to each element - .x 
  mutate(lm = map(data, ~lm(data = .x,
...
```{r}
 ...
 mutate(splot = map2(data,r2,
  ...
  # Making scatterplots with r2 in subtitle, and correct x and y axis titles: USE PMAP to iterate ins
